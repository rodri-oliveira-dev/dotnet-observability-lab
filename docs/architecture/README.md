...
**Interactive/static preview:** Run `npm run architecture:dev` locally, or `npm run architecture:build` to generate the relocatable site at `dist/architecture` with `--base ./`. The [Render and publish LikeC4 architecture workflow](https://github.com/rodri-oliveira-dev/dotnet-observability-lab/actions/workflows/architecture-preview.yml) validates the model, generates all seven PNGs with the same pinned LikeC4 version, checks the relocatable site assets and uploads `likec4-rendered-architecture` (PNG previews + static site) as a downloadable run artifact. For GitHub Pages, the workflow performs a separate build after validation: `actions/configure-pages` supplies the repository `base_path`, and LikeC4 builds `dist/pages` with that path before upload and deployment. This is required because a project Pages site is served below `/dotnet-observability-lab/`, while the local/downloadable build remains relocatable. On successful pushes to `main`, the Pages-specific build is deployed to [GitHub Pages](https://rodri-oliveira-dev.github.io/dotnet-observability-lab/). The one-time repository prerequisite is **Settings → Pages → Build and deployment → Source → GitHub Actions**; after that, architecture changes merged to `main` publish automatically. A `404 Not Found` from the `Deploy GitHub Pages` job means Pages has not been enabled for the repository yet; the default workflow `GITHUB_TOKEN` cannot perform that repository-administration setup. Enable the source once, then rerun the workflow (or use `workflow_dispatch`). Download/unzip the CI artifact; a normal extraction contains top-level `architecture/` and `architecture-previews/` directories. Serve the extracted `architecture/` directory with a local HTTP server (for example, `python3 -m http.server 8000 -d architecture`) when an offline copy is useful. Artifacts have limited retention; the committed Markdown gallery remains the durable GitHub-readable fallback.
